                            ...
                            <div style="flex: 1;">
                                ...
                                <div id="availableItemsList" style="height: 300px; overflow-y: auto; margin-right: 2em;">
                                    @foreach ($items as $item)
                                        @if (!in_array($item->id, $trainer->array_item_ids))
                                            <div>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        document.querySelectorAll('input[type="checkbox"]').forEach(function (checkbox) {
            checkbox.addEventListener('change', function () {
                ...
                moveItem(item, this.checked); // Move the corresponding item based on the checked status of the checkbox
            });
        });
        ...
        // Event listener for input fields (quantity)
        document.querySelectorAll('input[type="number"]').forEach(function (input) {
            input.addEventListener('input', function () {
                const itemId = this.id.split('_')[1];
                const index = arrayItemIds.indexOf(itemId);
                if (index !== -1) {
                    arrayQty[index] = this.value;
                } else {
                    // If the index is not found, it means this is a new item
                    arrayItemIds.push(itemId);
                    arrayQty.push(this.value);
                }
            });
        });
    });
